...
// ============================================
// DISPLAY MANAGER CLASS (3-second cycle + BPM shown immediately)
// ============================================
...
void setup() {
  ...
  Serial.println("[SETUP] MAX30105 found");
  ...
  Serial.println("[SETUP] Diaper: 1 min | Feeding: 2.5 min | LCD: 3 sec");
}
...
void loop() {
  ...
  if (millis() - lastDebug > 1200) {
    lastDebug = millis();
    Serial.print("IR="); Serial.print(state.irValue);
    Serial.print(" Finger="); Serial.print(state.fingerDetected ? "YES" : "NO");
    Serial.print(" BPM="); Serial.print(state.aveBPM, 1);
    Serial.print(" Status="); Serial.println(state.currentEmotion == 0 ? "CALM" : (state.currentEmotion == 1 ? "HAPPY" : "SAD"));
  }
}
